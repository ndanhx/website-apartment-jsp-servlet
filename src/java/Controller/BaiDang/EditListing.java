 ...
 * @author Home
 */
@WebServlet(name = "EditListing", urlPatterns = {"/edit-listing"})
public class EditListing extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request,
        ...
        int check = baidangdao.EditListing(tieu_de, idbaidang);
        if(check > 0){
             String message = "Chỉnh sửa bài đăng thành công!";
            request.setAttribute("success", message);
            request.getRequestDispatcher("GetAllBaiDang").forward(request, response);
        }else{
            
            String message = "Chỉnh sửa bài đăng không thành công!";
